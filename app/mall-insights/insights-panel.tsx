...
/**
 * MallInsightsPanel — read-only mall key numbers (GMV, products, liability,
 * event outbox, supplier payables). Rendered by BOTH the PontoSys page
 * (/mall-insights) and the PontoMall back-office 数据洞察 tab, so the logic
 * lives exactly once (same pattern as PointsEconomyPanel).
 * `hideAdminLinks` hides the "打开商城后台" shortcuts when the panel is
 * already embedded inside that back office.
 */
...
export default function MallInsightsPanel({ hideAdminLinks = false }: { hideAdminLinks?: boolean }) {
  ...
  const booting = loading && !ops && products.length === 0;
  const n = (value: string) => (booting ? "…" : value);
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Stat label="GMV 折算（R$）" value={n(`R$ ${(summary?.gmvBRL ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`)} hint={t("dynUnifiedBasis", { r: summary?.pointsToBrlRate ?? 10 })} />
        <Stat label="积分 GMV" value={n(`${(summary?.pointsGmv ?? 0).toLocaleString()} ${t("dynPts")}`)} hint="骑手累计消耗" />
        <Stat label="现金 GMV" value={n(`R$ ${(summary?.cashGmv ?? 0).toFixed(2)}`)} hint="PIX 补差实收" />
        <Stat label="待核销收款" value={n(String(summary?.pendingPayments ?? 0))} hint="商城后台处理" />
      </div>
      <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
        <Stat label="在售商品" value={n(String(products.filter((product) => product.status === "active").length))} hint={t("dynSkuCount2", { n: products.length })} />
        <Stat label="待定价" value={n(String(products.filter((product) => product.status === "pending_pricing").length))} hint="供应商已提报" />
        <Stat label="待付供应商" value={n(`R$ ${payablePending.toFixed(2)}`)} hint="已确认对账单" />
        <Stat label="供应商数" value={n(String(new Set(products.map((product) => product.supplierName).filter(Boolean)).size))} hint="有商品的供应商" />
      </div>
      <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
        <Stat label="高价值待审核" value={n(String(summary?.reviewPending ?? 0))} hint="商城后台处理" />
        <Stat label="合作方兑换" value={n(String(summary?.partnerOrders ?? 0))} hint="Partner 兑换单数" />
        <Stat label="合作方积分消耗" value={n(`${(summary?.partnerPointsSpent ?? 0).toLocaleString()} ${t("dynPts")}`)} hint="Partner 独立积分口径" />
        <Stat label="近 30 天兑换" value={n(String((summary?.daily ?? []).reduce((sum, day) => sum + day.count, 0)))} hint="最近 30 天合计" />
      </div>
      ...
      <div className="mt-5">
        ...
        >
          {hideAdminLinks ? (
            <span className="text-[11px] font-bold text-[var(--muted)]">只读关键数据 —— 商品、订单、收款与结算的操作在各业务 Tab 完成。</span>
          ) : (
            <>
