    ...
    <AppShell>
      ...
      <section className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div>
          <div className="mb-3 flex items-center gap-2">
            <FileBarChart2 className="text-[var(--accent)]" size={20} />
              ...
              <div key={item.id} className="rounded border border-[var(--line)] bg-[var(--surface)] p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-[var(--muted)]">{item.owner}</div>
                  </div>
                  <Badge value={item.status} />
                </div>
                <div className="mt-3 grid gap-2 text-xs text-[#a5a5bd]">
                  <div>
                    <span className="font-black text-[var(--text-soft)]">BigQuery:</span> {item.bigQuery}
                  </div>
                  <div>
                    <span className="font-black text-[var(--text-soft)]">Redshift:</span> {item.redshift}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[380px_minmax(0,1fr)]">
        <div className="panel p-4">
          <div className="mb-3 text-xs font-black uppercase text-[var(--accent)]">KPI snapshots</div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            {warehouseSummary.kpis.map((kpi) => (
              <div key={kpi.id} className="rounded border border-[var(--line)] bg-[var(--surface)] p-3">
                <div className="text-xs uppercase text-[var(--muted)]">{kpi.label}</div>
                <div className="mt-2 flex items-end justify-between gap-3">
                  <div className="text-3xl font-black">{kpi.value}</div>
                  <div className="rounded bg-[var(--accent)]/15 px-2 py-1 text-xs font-black text-[var(--accent)]">{kpi.delta}</div>
                </div>
                <div className="mt-2 text-xs text-[var(--muted)]">{kpi.source}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="mb-3 flex items-center gap-2">
            <CalendarClock className="text-[var(--accent)]" size={20} />
      ...
      <section className="mt-5 rounded border border-[var(--line)] bg-[var(--surface)] p-4">
        <div className="text-xs font-black uppercase text-[var(--accent)]">Next migration milestone</div>
