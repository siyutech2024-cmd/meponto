...
function RuleDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[var(--overlay)] backdrop-blur-sm">
      <aside className="ml-auto flex h-full w-full max-w-xl flex-col border-l border-[var(--line)] bg-[var(--surface)] shadow-2xl">
        <div className="flex min-h-16 items-center justify-between border-b border-[var(--line)] px-4">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="grid h-10 w-10 place-items-center rounded border border-[var(--line)] bg-[var(--surface-raised)] text-[var(--text-soft)] hover:border-[var(--accent)] hover:text-[var(--text)]"
          >
            <X size={18} />
          </button>
        </div>
        <div className="overflow-y-auto p-4">
          ...
          >
            <label className="block">
              <span className="mb-2 block text-xs font-black uppercase text-[var(--muted)]">Rule Name</span>
              <input
                name="ruleName"
                required
                defaultValue={rule?.ruleName ?? ""}
                className="h-11 w-full rounded border border-[var(--line)] bg-[var(--surface-raised)] px-3 outline-none"
              />
            </label>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase text-[var(--muted)]">Points</span>
                <input
                  name="points"
                  required
                  min="1"
              ...
              <label className="block">
                <span className="mb-2 block text-xs font-black uppercase text-[var(--muted)]">Type</span>
                <select
                  name="type"
        ...
        <div className="panel p-4">
          ...
          <div className="space-y-3">
            {riders
              ...
              .slice(0, 3)
              .map((rider, index) => (
                <div key={rider.id} className="flex items-center justify-between rounded border border-[var(--line)] bg-[var(--surface)] p-3">
                  <div>
                    <div className="font-black">{index + 1}. {rider.name}</div>
                    <div className="text-sm text-[var(--muted)]">{rider.ponto}</div>
                  </div>
                  <Badge value={`${rider.ar}% AR`} />
                </div>
              ))}
          </div>
          <h3 className="mb-3 mt-5 text-sm font-black uppercase text-[var(--muted)]">Top Leaders</h3>
          <div className="space-y-3">
            {leaders.map((leader) => (
              <div key={leader.id} className="flex items-center justify-between rounded border border-[var(--line)] bg-[var(--surface)] p-3">
