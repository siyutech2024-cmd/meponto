...
/**
 * PartnerPointsPanel — 合作方积分 workbench body (register a rider service,
 * partner ledger/balance, service records table). Rendered by BOTH the
 * PontoSys page (/partner-points) and the PontoMall back-office 合作方积分 tab
 * (app/mall/tabs/partner-points.tsx) — one implementation, two homes (same
 * pattern as MembersPanel / MallInsightsPanel). `hideAdminLinks` hides the
 * "→ CRM" shortcut when the panel is already embedded inside the mall back
 * office (CRM is a sibling tab there).
 */
...
export default function PartnerPointsPanel({ hideAdminLinks = false }: { hideAdminLinks?: boolean }) {
  const [services, setServices] = useState<PartnerServiceRecord[]>([]);
  /** True until the first load settles — stats show "…" + table shows Skeleton. */
  ...
  const booting = loading && services.length === 0;
  ...
  return (
    <div>
      {/* Panel-level action (was the PageTitle action slot — moved in so both
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <Stat label="Serviços" value={booting ? "…" : String(services.length)} />
        <Stat label="Confirmados" value={booting ? "…" : String(services.filter((item) => item.status === "confirmed").length)} />
        <Stat label="Em análise" value={booting ? "…" : String(services.filter((item) => item.status === "pending").length)} />
