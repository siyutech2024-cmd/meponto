...
/**
 * Shared premium digital business card (used by /liuming and /ishak).
 * Tri-lingual switcher (sliding-thumb pill in the yellow header), WhatsApp QR
 * modal, tel/mail/site/maps links, vCard download, Web Share, QR codes.
 * Brand: #0b0e14 · #FFD400. Wrapped in data-i18n-skip — the card owns its i18n.
 */

export type Lang = "pt" | "zh" | "en";

export type EcardPerson = {
  nameZh: string;
  nameEn: string;
  /** Role line per language, e.g. "Fundador · meponto". */
  role: Record<Lang, string>;
  email: string;
  /** Display format, e.g. "+86 18686514086". */
  phone: string;
  whatsappUrl: string;
  avatar: string;
  wechatQr: string;
...
const ADDRESS_TEXT = "Av. Paulista, 2537 · Bela Vista, São Paulo";
const ADDRESS_URL =
...
export default function BusinessCard({ person }: { person: EcardPerson }) {
  ...
  const saveContact = () => {
    const blob = new Blob([person.vcard], { type: "text/vcard;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = person.vcfFileName;
  ...
  const shareCard = async () => {
    const data = { title: `${person.nameEn} · meponto`, text: "O ponto de quem entrega — meponto", url: window.location.href };
    if (navigator.share) {
  ...
  const tiles = [
    ...
    { icon: ICONS.wechat, label: t.wxLab, value: person.phone, href: `tel:${person.phone.replace(/\s/g, "")}` },
    { icon: ICONS.mail, label: t.emailLab, value: person.email, href: `mailto:${person.email}` },
    { icon: ICONS.globe, label: t.siteLab, value: "meponto.com", href: SITE, external: true },
    { icon: ICONS.pin, label: t.addrLab, value: ADDRESS_TEXT, href: ADDRESS_URL, external: true, wide: true },
  ];
  ...
  return (
    <main data-i18n-skip className="relative flex min-h-screen items-start justify-center overflow-hidden px-4 py-8 md:items-center md:py-14" style={{ background: "#0b0e14", color: "#fff7ef", fontFamily: "Outfit, Inter, system-ui, sans-serif" }}>
          ...
          <div className="overflow-hidden rounded-[29px]" style={{ background: "#11141f" }}>
            ...
            <div className="flex flex-col gap-6 px-7 pb-8 pt-6">
              <div className="lm-rise-2">
                <div className="text-[26px] font-black leading-none">{person.nameZh}</div>
                <div className="mt-2 text-[12px] font-black uppercase tracking-[0.14em]" style={{ color: "#FFD400" }}>{person.nameEn}</div>
                <div className="mt-1.5 text-[11px] font-bold" style={{ color: "rgba(255,255,255,.55)" }}>{person.role[lang]}</div>
              </div>
              ...
              <div className="lm-rise-4 flex justify-center gap-4">
                {[
                  { src: person.wechatQr, title: "WeChat · 微信", cap: t.wcCap },
                  { src: person.vcardQr, title: "vCard", cap: t.vcCap },
                ].map((qr) => (
        ...
        >
          <div className="relative w-full max-w-[320px] rounded-3xl bg-white p-6 text-center" style={{ color: "#0b0e14" }}>
            ...
            <div className="mx-auto mt-4 w-[210px]">
              <img src={person.whatsappQr} alt="WhatsApp QR" className="aspect-square w-full object-contain" />
            </div>
            <a
      ...
      {/* toast */}
      <div
        className="fixed bottom-8 left-1/2 z-[60] -translate-x-1/2 rounded-xl text-[13px] font-black transition-all duration-200"
