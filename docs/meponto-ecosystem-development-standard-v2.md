...
本文档是 MePonto / PontoSys 从第一天按大型系统建设的开发规范。它用于约束主后台、加盟商后台、Leader 工作台、骑手端 App、Partner 后台、供应链后台、积分商城、宠物养成系统及后续新增模块的架构、代码、数据、权限、测试、上线和多人协作方式。
...
- 如何支持巴西本地化运营、LGPD 数据合规、PIX 风控和 WhatsApp 驱动的沟通体系。
...
骑手端、Partner、积分商城、后台审核、风控、财务、分析、游戏化任务都必须遵守同一套积分账本、限制、防刷、过期、兑换和退款规则。任何模块不得单独定义另一套积分余额或积分消费规则。
...
库存变化必须进入 `inventory_ledger`。商城只能 reserve / deduct，不允许直接改库存余额。
...
### 27A.3 WhatsApp 作为核心通信基础设施

在巴西，WhatsApp 不是普通通知渠道，而是核心运营基础设施。
...
- 新增巴西本地化合规：LGPD、PIX 风控、WhatsApp 作为核心通信基础设施。
